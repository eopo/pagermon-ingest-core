...
class HealthMonitor {
  ...
  constructor(config, options = {}) {
    ...
    this.targetName = String(config.targetName || 'default');
  ...
  async perform() {
    try {
      ...
      if (healthy) {
        ...
        this.failureCount++;
        this.failureCounter.labels({ target_name: this.targetName }).inc();
      ...
      // Update health gauge
      this.healthGauge.labels({ target_name: this.targetName }).set(this.isHealthy ? 1 : 0);
      ...
      if (this.failureCount >= this.unhealthyThreshold && this.isHealthy) {
        this.isHealthy = false;
        this.healthGauge.labels({ target_name: this.targetName }).set(0);
