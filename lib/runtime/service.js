...
/**
 * Start the ingest service and manage its lifecycle, including metrics, per-target API clients, queue,
 * worker processing, adapter orchestration, health monitoring, and graceful shutdown.
 *
 * @param {Object} [options] - Dependency injection overrides.
 * @param {Function} [options.adapterFactory] - Optional factory used to create adapters for the orchestrator.
 * @param {Function|Class} [options.ApiClientClass=ApiClient] - Constructor used to create per-target API clients.
 * @param {Function|Class} [options.QueueManagerClass=QueueManager] - Constructor used to create the queue manager.
 * @param {Function|Class} [options.HealthMonitorClass=HealthMonitor] - Constructor used to create per-target health monitors.
 * @param {Function|Class} [options.WorkerClass=Worker] - Constructor used to create the worker that processes queue messages.
 */
      ...
      async (incomingMessage) => {
        try {
          ...
          if (failedTargets.length > 0) {
            throw new Error(`Failed to enqueue message for targets: ${failedTargets.join(',')}`);
          }
        } catch (err) {
